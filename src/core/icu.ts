...
export interface IcuResolveContext {
  ...
  formatter?: Formatter;
  /** Called when a required ICU placeholder variable is missing from the params. */
  onError?: (error: I18nError) => void;
}

/**
 * Parses cases in ICU expressions (e.g. '=0 {No items} one {# item} other {# items}').
 */
...
/**
 * Resolves ICU syntax (select, plural, selectordinal, number, date, time, and variables) in a template string.
 */
      ...
      if (depth === 0) {
        ...
        if (commas.length >= 2) {
          ...
          if (formatType === "select") {
            ...
            result += resolveIcu(branch, params, lang, context);
            ...
            result += resolveIcu(branchWithNum, params, lang, context);
          ...
          if (formatType === "number" && context.formatter) {
            ...
            result += val !== undefined ? String(val) : fullExpr;
          }
        } else {
          ...
          if (val !== undefined && typeof val !== "function") {
            ...
            // Variable missing from params — report and keep the raw expression as fallback.
            if (params !== undefined) {
              context.onError?.(
                new I18nError(
