...
describe("Missing Key Lifecycle & Custom Fallback", () => {
  ...
  it("supports custom onMissingKey and missingKeyHandler", () => {
    const reportedKeys: Array<{ key: string; lang: string }> = [];

    const customI18n = createI18n({
      defaultLanguage: "en",
      messages: { en: { welcome: "Welcome" } as const },
      onMissingKey: (key, lang) => {
        reportedKeys.push({ key, lang });
      },
      missingKeyHandler: (key, lang) => `[FALLBACK:${lang}:${key}]`,
    });
    ...
    expect(reportedKeys).toEqual([{ key: "header.subtitle", lang: "en" }]);
